...
Deploy: Render.com | Data: GitHub-hosted CSV
"""
...
for col in df.columns:
    if "track" in col and "name" in col:    rename_map[col] = "track_name"
    if col == "artists":                     rename_map[col] = "artist"
    if "artist" in col and col != "artist": rename_map[col] = "artist"
    if "track_genre" == col:                rename_map[col] = "genre"
    if col == "genre":                       rename_map[col] = "genre"
...
# ── Fallback: if genre column missing, create one ──
if "genre" not in df.columns:
    df["genre"] = "Unknown"

print(f"✅ Loaded {len(df):,} tracks | {df['genre'].nunique()} genres")
...
def section(children, extra=None):
...
app.layout = html.Div(style={"backgroundColor": BG, "minHeight": "100vh",
                               "fontFamily": "'Courier New', monospace", "color": TEXT}, children=[
    ...
    html.Div([
        ...
        html.Div([
            ...
            html.P(f"Exploratory Analysis · {len(df):,} Tracks · {df['genre'].nunique()} Genres",
    ...
    html.Div(style={"padding": "24px 36px"}, children=[
        ...
        # Row 1
        html.Div([
            section([dcc.Graph(id="genre-bar",  style={"height": "300px"})],
                    {"flex": "1", "minWidth": "300px", "marginBottom": "0"}),
            section([dcc.Graph(id="year-line",  style={"height": "300px"})],
                    {"flex": "1", "minWidth": "300px", "marginBottom": "0"}),
        ], style={"display": "flex", "gap": "18px", "marginBottom": "18px"}),

        # Row 2
        html.Div([
            section([dcc.Graph(id="radar-chart",  style={"height": "340px"})],
                    {"flex": "1", "minWidth": "300px", "marginBottom": "0"}),
            section([dcc.Graph(id="scatter-plot", style={"height": "340px"})],
                    {"flex": "1.4", "minWidth": "340px", "marginBottom": "0"}),
        ], style={"display": "flex", "gap": "18px", "marginBottom": "18px"}),

        # Row 3
        html.Div([
            section([dcc.Graph(id="corr-heatmap", style={"height": "340px"})],
                    {"flex": "1.2", "minWidth": "320px", "marginBottom": "0"}),
            section([dcc.Graph(id="box-plot",     style={"height": "340px"})],
                    {"flex": "1",   "minWidth": "300px", "marginBottom": "0"}),
        ], style={"display": "flex", "gap": "18px", "marginBottom": "18px"}),
...
def update(genre, year_range, min_pop):
    ...
    if fdf.empty:
        ...
        return [], empty, empty, empty, empty, empty, empty, html.P("No data.", style={"color": SUBTEXT})
    ...
    fig1.update_layout(showlegend=False,
                        xaxis=dict(tickangle=-30, gridcolor=GRIDCOLOR),
                        height=300)
    ...
    # ── Radar ────────────────────────────────
    rf = [f for f in ["danceability","energy","valence","acousticness","speechiness"] if f in fdf.columns]
    radar_colors = ["#1DB954","#FF6B6B","#4ECDC4","#FFE66D","#A29BFE"]
    fig3 = go.Figure()
    genres_for_radar = fdf["genre"].value_counts().head(5).index if genre == "All" else [genre]
    for i, g in enumerate(genres_for_radar):
    ...
    num_cols = [c for c in ["popularity","danceability","energy","valence",
                             "tempo","loudness","speechiness","acousticness"] if c in fdf.columns]
    ...
    table = dash_table.DataTable(
        ...
        style_data_conditional=[
            {"if": {"row_index": 0}, "backgroundColor": "rgba(29,185,84,0.08)", "color": GREEN2},
